function ValidaCampos (form) {
  const expresion_regular_dni = /^\d{2}[.]\d{3}[.]\d{3}[-][A-Z]{1}$/

  const expresion_regular_tel = /[+34][ ]\d{9}$/

  const expresion_regular_hora = /^(0[1-9]|1\d|2[0-3]):([0-5]\d)$/

  const nombre = document.getElementById('nombre').value.split(' ')

  const apellido = document.getElementById('apellidos').value.split(' ')
